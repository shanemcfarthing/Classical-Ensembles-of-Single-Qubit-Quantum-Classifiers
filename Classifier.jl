...
function train_learner(train_X, valid_X, config, nfevs, depth)
...
# uses the base learners in the ensemble to classify the datasets datapoints, and combines
# the predictions to create the metalearner's dataset.
function combine_learner_predictions(dataset, learner_weights::Vector{Vector{Float64}}, depth)

    # holds the learner classifications
    learners_predictions::Vector{Vector} = [[] for i in 1:length(learner_weights)]
    
    # for each of the learners, classify the entire dataset
    for (index, i) in collect(enumerate(learner_weights))

        # create the quantum circuit
        var_circ = get_var_circuit("QAUM"; depth=depth)
        nparams = nparameters(var_circ)
        initial_state = zero_state(1)
        ...
        for j in dataset
            ...
            # create the full parameter set for the quantum circuit 
            circ_params = get_circuit_params(j[1:8], i, nparams)
            # bind the parameters to the circuit
            dispatch!(var_circ, circ_params)

            # get the class probabilities for the datapoint
            probs = circuit_predict(var_circ, copy(initial_state))
            # only record the probability of label 1
            push!(probabilities, probs[2])
        end
        # use the class probabilities for every instance in the dataset to get the classification
        #classifications = classify_from_probs(probabilities)
        #push!(learners_predictions, classifications)

        learners_predictions[index] = probabilities
    end
    
    # process the individual learner predictions to create the metalearner's training dataset.
    # Element i in pred_features is a list of each base learner's prediction of datapoint i in
    # the original dataset.
    pred_features = [[] for i in 1:length(dataset)]

    for i in learners_predictions
        count = 1
        for j in i
            push!(pred_features[count],j)
            count += 1
        end
    end

    # return the metalearner dataset
